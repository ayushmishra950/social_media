...
const RegisterForm = () => {
  ...
  useEffect(() => {
    ...
    // Cleanup function - reset form when unmounting if not going to OTP
    return () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save form data before navigating to OTP
      localStorage.setItem('registerFormData', JSON.stringify(formData));
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                required
                minLength={6}
                className={`mt-1 w-full px-4 py-2 border ${passwordError ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 ${passwordError ? 'focus:ring-red-400' : 'focus:ring-blue-400'} pr-10`}
              />
              <button
                type="button"
