  ...
  const [isLoadingProfile, setIsLoadingProfile] = useState(true);
     ...
     const user = async() => {
      ...
      try{
        ...
        const isBlockedUser = user?.name === "Unknown User" || user?.username === "unknown_user";
    ...
    const profileData = {
      ...
      isBlockedUser: isBlockedUser // Add blocked user flag
    };
...
// window.location.reload();      
  ...
  // Listen for follow/unfollow events to update profile in real-time
  useEffect(() => {
    const handleFollowUpdate = (event) => {
      ...
      if (userId === profile.id) {
        console.log(`🔄 Main.js received follow update: ${action} for user ${userId}`);
        
        // Refetch user data to get latest counts
        if (tokens) {
          user();
        }
      }
    ...
    window.addEventListener('profileUpdated', handleFollowUpdate);
    
    return () => {
      window.removeEventListener('profileUpdated', handleFollowUpdate);
    };
  }, [profile.id, tokens]);
  ...
  useEffect(() => {
    if (videosData?.getUserVideos) {      
      console.log('📹 User videos fetched:', videosData.getUserVideos);
      console.log('📹 Videos count:', videosData.getUserVideos.length);
      setUserVideos(videosData.getUserVideos); // set user videos for reels
    } else {
      console.log('📹 No videos data received:', videosData);
    }
  }, [videosData]);
  ...
  useEffect(() => {
    const handlePostUploaded = () => {
      console.log('📤 Post uploaded event received, refetching data...');
      if (typeof refetch === 'function') {
        refetch().then(() => console.log('✅ Posts refetched successfully'));
      }
      if (typeof refetchVideos === 'function') {
        refetchVideos().then(() => console.log('✅ Videos refetched successfully'));
      }
    };
