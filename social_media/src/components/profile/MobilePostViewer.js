...
const MobilePostViewer = ({ posts, initialIndex = 0, onClose, currentUser }) => {
  ...
  const handleArchive = (postId) => {
    /* console.log(...) */ void 0;
    // TODO: Add API call to archive post
    ...
    return (
      <div className="w-full bg-white flex items-start justify-center p-2 relative">
        ...
        <div className="w-full max-w-sm bg-white rounded-xl shadow-md overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-3 py-2 pr-16">
            ...
            <div className="relative mr-4">
              <button 
                ...
                onClick={() => setShowDropdown(prev => ({ ...prev, [postId]: !prev[postId] }))}
              >
                <BsThreeDots />
              </button>
              {showDropdown[postId] && (
                <div className="absolute right-0 mt-2 w-44 bg-white border border-gray-200 rounded-xl shadow-lg z-30 overflow-hidden">
                  <button
                    className="flex items-center w-full px-4 py-2 text-gray-700 hover:bg-gray-50 text-sm font-medium"
                    onClick={() => handleArchive(postId)}
                  >
                    Archive
                  </button>
                  <button
                    className="flex items-center w-full px-4 py-2 text-purple-600 hover:bg-purple-50 text-sm font-medium border-t border-gray-100"
                    onClick={() => { setShowDropdown(prev => ({ ...prev, [postId]: false })); handleSave(postId); }}
                  >
                  ...
                  {isOwner && (
                    <button
                      ...
                      onClick={() => { setShowDropdown(prev => ({ ...prev, [postId]: false })); handleDeletePost(); }}
                    >
