...
import { LIKE_POST, COMMENT_POST, REPLY_TO_COMMENT, LIKE_COMMENT, LIKE_REPLY, DELETE_COMMENT, ARCHIVE_POST, UNARCHIVE_POST } from '../../graphql/mutations';
...
const PostViewer = ({ posts, initialIndex = 0, onClose, currentUser }) => {
  ...
  // Check if post is archived (optional: if you have this info in post data)
  useEffect(() => {
    if (currentPost && typeof currentPost.isArchived === 'boolean') {
      setIsArchived(currentPost.isArchived);
    }
  }, [currentPost]);
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      if (Array.isArray(serverComments)) {
        setCommentsList(prev => {
          // Build a reference users map from both previous state and current post
          const refUsers = new Map();
          const pushUser = (u) => {
            ...
            if (!refUsers.has(uid)) {
              refUsers.set(uid, {
                ...
                username: u?.username || (u?.name ? String(u.name).toLowerCase() : 'user'),
                profileImage: u?.profileImage || u?.avatar || u?.image || null,
              });
            }
          };
          prev.forEach(p => { if (p?.user) pushUser(p.user); (Array.isArray(p?.replies) ? p.replies : []).forEach(r => r?.user && pushUser(r.user)); });
          (Array.isArray(currentPost?.comments) ? currentPost.comments : []).forEach(p => { if (p?.user) pushUser(p.user); (Array.isArray(p?.replies) ? p.replies : []).forEach(r => r?.user && pushUser(r.user)); });

          // Normalize server comments ONLY (replace list to avoid duplicates)
          const getUserId = (u) => u && typeof u === 'object' ? (u.id || u._id) : (u || '');
          const sigOf = (obj) => `${String(obj?.text || '')}|${String(getUserId(obj?.user) || obj?.userId || obj?.uid || '')}`;
          const prevBySig = prev.reduce((acc, item) => { const s = sigOf(item); if (s) acc[s] = item; return acc; }, {});

          const normalized = serverComments.map(sc => {
            const serverUser = sc.user || sc.createdBy || sc.commentedBy || sc.userId || sc.uid;
            const isObjUser = serverUser && typeof serverUser === 'object';
            const isIdUser = serverUser && typeof serverUser === 'string';

            let resolvedUser;
            if (isObjUser) {
              const suid = String(serverUser.id || serverUser._id || '');
              const ref = suid ? refUsers.get(suid) : null;
              resolvedUser = {
                id: suid || (ref?.id) || (prev[0]?.user?.id) || validUserId,
                name: serverUser.name || serverUser.fullName || ref?.name || currentUser?.name || user?.name || 'User',
                username: serverUser.username || serverUser.userName || ref?.username || ((serverUser.name || 'user') + '').toLowerCase(),
                profileImage: serverUser.profileImage || serverUser.avatar || serverUser.image || ref?.profileImage || currentUser?.profileImage || user?.profileImage || null,
              };
            } else if (isIdUser) {
              const ref = refUsers.get(String(serverUser));
              resolvedUser = ref || {
                id: String(serverUser),
                name: currentUser?.name || user?.name || 'User',
                username: currentUser?.username || user?.username || 'user',
                profileImage: currentUser?.profileImage || user?.profileImage || null,
              };
            } else {
              resolvedUser = currentUser || user || { id: validUserId, name: 'User', username: 'user', profileImage: null };
            }

            // Reuse previous id if server lacks it (prevents new synthetic id on each fetch)
            const scSig = sigOf({ ...sc, user: isObjUser ? serverUser : { id: serverUser } });
            const prevMatch = prevBySig[scSig];
            const normalizedId = sc.id || sc._id || sc.commentId || sc.cid || prevMatch?.id || `c-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
            return { ...prevMatch, ...sc, id: normalizedId, user: resolvedUser };
          });

          // Dedupe by signature to avoid duplicates if server returns duplicates
          const seen = new Set();
          const unique = [];
        ...
        /* console.log(...) */ void 0;
      } else if (serverComments && typeof serverComments === 'object') {
        // Single comment returned: merge it in place of the optimistic one
        setCommentsList(prev => {
          const sc = serverComments || {};
          const su = sc.user || {};
          const merged = {
            id: sc.id || newComment.id,
            text: sc.text || newComment.text,
            user: {
              id: su.id || newComment.user?.id || validUserId,
              name: su.name || newComment.user?.name || 'User',
              username: su.username || newComment.user?.username || 'user',
              profileImage: su.profileImage || newComment.user?.profileImage || null,
            },
            commentedAt: sc.commentedAt || newComment.commentedAt,
          };
          const updated = prev.map(c => c.id === newComment.id ? merged : c);
          const pid = String(currentPost.id);
          postOverridesRef.current[pid] = {
            ...(postOverridesRef.current[pid] || {}),
            isLiked,
  ...
  const handleCommentLike = async (commentId) => {
    ...
    try {
     const cm = await likeComment({ variables: { userId: validUserId, postId: validPostId, commentId: validCommentId } });
     /* console.log(...) */ void 0;
    } catch (e) {
      ...
      >
        ...
        <div className="flex flex-col md:grid md:grid-cols-[auto_20rem] w-full h-full min-h-0">
          ...
          <div className="w-full md:w-[20rem] md:shrink-0 md:h-full bg-white flex flex-col min-h-0 border-l-0 md:border-l border-gray-200 border-t md:border-t-0 max-h-[45vh] md:max-h-none">
            {/* Header */}
            <div className="flex items-center justify-between p-3 border-b border-gray-200 pr-16">
                ...
                {showDropdown && (
                  <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-20 min-w-[120px]">
                    <button
                      ...
                      className={`w-full px-4 py-2 text-left text-sm transition-colors duration-200 ${isArchived ? 'text-green-600' : 'text-gray-700 hover:bg-gray-100'}`}
                    >
