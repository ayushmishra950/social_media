...
const NotificationBell = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Check if the click is outside the notification bell and dropdown
      if (bellRef.current && !bellRef.current.contains(event.target)) {
        ...
        // Only close the dropdown if not clicking a navigation link
        if (!isNavLink) {
          setIsOpen(false);
        } else {
          // If it is a navigation link, close the dropdown after a small delay
    ...
    setTimeout(() => {
      ...
      if (notification.type === 'comment' || notification.type === 'like' || notification.type === 'comment_like') {
        const state = {
          ...
          highlightComment: notification.type === 'comment' || notification.type === 'comment_like',
          highlightLike: notification.type === 'like',
          commentId: notification.commentId
        };
        
        // Navigate to home with the post ID in state
        navigate('/', { 
          state: state,
          replace: true // Replace current entry in history
        });
      } else if (notification.type === 'follow' && notification.sender?.id) {
        // Navigate to the profile of the user who followed
        navigate(`/profile/${notification.sender.id}`, { replace: true });
      }
    }, 100);
  };
