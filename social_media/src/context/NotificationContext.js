...
export const NotificationProvider = ({ children }) => {
    ...
    setNewNotifications(prev => {
      ...
      const filtered = prev.filter(notification => {
        try {
          ...
          // If there's an error parsing the date, keep the notification
          return true;
        }
      });
  ...
  }, []); // No dependencies needed as we only use setState
  ...
  useEffect(() => {
    // Only run cleanup if there are notifications to process
    if (newNotifications.length > 0) {
      cleanupOldNotifications();
    }
    
    // Set up interval to clean up every hour, but only if there are notifications
  ...
  }, [cleanupOldNotifications, newNotifications.length]);
